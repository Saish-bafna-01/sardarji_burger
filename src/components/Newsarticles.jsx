          ...
          {articles.map((article, index) => (
            ...
            >
              ...
              <div className="relative w-full aspect-[4/3] overflow-hidden rounded-sm group/img">
                ...
                <span className="absolute bottom-0 left-0 bg-[#c0d4b2] text-black text-xs sm:text-sm font-semibold px-3 sm:px-4 py-1 transition-all duration-300 group-hover/img:bg-[#c0d4b2]-300 group-hover/img:px-5">
                  {article.category}
                </span>
                {/* Arrow icon on hover */}
                <div className="absolute top-4 right-4 w-10 h-10 bg-[#c0d4b2] rounded-full flex items-center justify-center opacity-0 translate-x-4 group-hover/img:opacity-100 group-hover/img:translate-x-0 transition-all duration-500">
              ...
              <div className="flex flex-col gap-2 md:gap-3 pt-4 md:pt-5">
                <h3 className="text-white oswald text-lg sm:text-xl md:text-2xl font-bold uppercase leading-snug line-clamp-2 transition-colors duration-300 group-hover:text-[#c0d4b2]">
                  {article.title}
                </h3>
                ...
                <span className="flex items-center gap-1 text-[#c0d4b2] text-sm md:text-base font-semibold group-hover:gap-3 transition-all duration-300 mt-1 w-fit">
                  Read More{" "}
                  <FiArrowRight
                    size={14}
